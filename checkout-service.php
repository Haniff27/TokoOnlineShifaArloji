...
} elseif (empty($_SESSION["cart_service"])) {
...
<div class="banner mb-3">
    <div class="container-fluid img">
        <div class="container-fluid box">
            ...
            <p>Home > <a href="#">Service Checkout</a></p>
        </div>
    </div>
</div>
<div class="content">
    <div class="container">
        <div class="row mb-3">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        <h4 class="mt-0 header-title">Data Service</h4>
                        ...
                        <form action="" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class=" form-control" value="" name="nama" id="nama" placeholder="nama lengkap" required>
                            </div>
                            <div class="form-group">
                                <label class="font-weight-bold">No Telp</label>
                                <input type="number" class=" form-control" value="" name="no_telp" placeholder="contoh: 085xxxx" required>
                            </div>
                            <div class="form-group">
                                <label class="font-weight-bold">Provinsi</label>
                                <select name="province_destination" id="province_destination" class="all_province form-control" onchange="get_city_destination(this);" required>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="textarea" class="form-control" maxlength="225" rows="3" placeholder="Masukkan alamat lengkap (nama jalan, nama dusun, Rt/Rw, dst)" name="alamat" required></textarea>
                            </div>
                            <div class="form-group">
                                <label for="" class="font-weight-bold">Catatan</label>
                                <textarea id="textarea" class="form-control" maxlength="225" rows="3" name="catatan"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="" class="font-weight-bold">Waktu Pengantaran</label>
                                <input type="datetime-local" class="form-control" name="waktu_pengantaran" required>
                            </div>
                            <div class="form-group">
                                <label for="" class="font-weight-bold">Deadline</label>
                                <input type="datetime-local" class="form-control" name="deadline" required>
                            </div>
                            <div class="form-group">
                                <label for="" class="font-weight-bold">Pilih Jenis Antar</label>
                                <select name="jenis_antar" id="" class="form-control">
                                    <option value="Antar Sendiri">Antar Sendiri</option>
                                    <option value="Lewat Kurir GoSend">Lewat Kurir GoSend</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <button class="btn btn-primary" name="pesan_service">Buat Pesanan</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-md-6">
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="mt-0 header-title">Keranjang Service</h5>
                                <p class="text-muted m-b-30 font-14">Lihat <a href="cart-service.php" style="text-decoration: none;">detail</a> detail
                                    keranjang service</p>
                                <table class="table" style="font-size: small;">
                                    ...
                                    <tbody>
                                        <?php
                                        $subtotal = 0;
                                        foreach ($_SESSION["cart_service"] as $id_service => $jumlah) : ?>
                                            <?php
                                            $query = "SELECT * FROM tbl_service WHERE id_service='$id_service'";
                                            $result = mysqli_query($db, $query);
                                            $service = mysqli_fetch_array($result);
                                            $subharga = $service['harga'] * $jumlah;
                                            ?>
                                            <tr>
                                                <td class="product-list-img">
                                                    <?php if ($service['gambar'] != null) : ?>
                                                        <img width="40" src="admin/assets/images/foto_service/<?php echo $service['gambar'] ?>" class="img-fluid" alt="tbl">
                                                    <?php endif; ?>
                                                </td>
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-sm-6 text-right">
                                        Rp. <span id="subtotal" name="subtotal"><?php echo number_format($subtotal); ?></span>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-sm-6">
                                        Biaya Pengiriman <span class="text-danger" style="font-size: 11px;">*(JNE EXPRESS)</span>
                                    </div>
                                    <div class="col-sm-6 text-right">
                ...
                <?php
                if (isset($_POST['pesan_service'])) {

                    $getAntar = $_POST['waktu_pengantaran'];
                    $waktu_pengantaran = date('Y-m-d H:i:s', strtotime($getAntar));
                    ...
                    $jenis_antar = $_POST['jenis_antar'];
                    $tgl_order = date('Y-m-d');
                    $ongkir = $_POST['ongkir'];
                    $totalbayar = $_POST['subtotal'];

                    //simpan data ke pelanggan
                    $query = "INSERT INTO tbl_order_service (id_pelanggan,nm_pelanggan,telp,provinsi,kota,kode_pos,alamat,catatan,tgl_order,waktu_pengantaran,deadline,jenis_antar,ongkir,total_order)
                            VALUES ('$id_pelanggan','$nmPenerima','$no_telp','$provinsi','$kota','$kdPos','$alamat','$catatan','$tgl_order','$waktu_pengantaran','$deadline','$jenis_antar','$ongkir','$totalbayar')";
                    $result = mysqli_query($db, $query);
                    //mengambil id_order barusan terjadi
                    $id_order_barusan = $db->insert_id;

                    //simpan detail_order
                    foreach ($_SESSION['cart_service'] as $id_service2 => $jumlah2) {
                        //mendapatkan data Barang berdasarkan id_barang
                        $ambil = "SELECT * FROM tbl_service WHERE id_service='$id_service2'";
                        $result2 = mysqli_query($db, $ambil);
                        $data = mysqli_fetch_array($result2);

                        $nm_service = $data['nm_service'];
                        $harga = $data['harga'];
                        $subharga2 = $harga * $jumlah2;

                        $query3 = "INSERT INTO tbl_detail_service (id_orderservice,id_service,nm_service,harga,jml_order,subharga)
                            VALUES ('$id_order_barusan','$id_service2', '$nm_service','$harga','$jumlah2','$subharga2' )";
                        $result3 = mysqli_query($db, $query3);

                        //update stok
                        $query4 = "UPDATE tbl_service SET stok=stok-$jumlah2 WHERE id_service='$id_service2'";
                    ...
                    echo "<meta http-equiv='refresh' content='1;url=pembayaran-service.php?id=$id_order_barusan'>";
